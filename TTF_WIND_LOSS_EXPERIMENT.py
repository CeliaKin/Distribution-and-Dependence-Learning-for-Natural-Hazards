# 1D TTF method on windstorm data
...
def _rqs_forward(x, widths, heights, derivatives, tail_bound):
    ...
    bin_idx = torch.searchsorted(
        cum_w[..., 1:-1].contiguous(), xc.unsqueeze(-1)
    ...
    xi    = ((xc - x_k) / (x_k1 - x_k + 1e-8)).clamp(0.0, 1.0)
    ...
    log_jac = torch.where(inside, lj_, torch.zeros_like(x))
    return z, log_jac
...
fig.suptitle(f"TTF on Windstorm Loss  (N = {len(x_all):,})",
             fontsize=13, fontweight="bold")
plt.tight_layout()
plt.savefig("ttfplot.png", dpi=150, bbox_inches="tight")
plt.show()
