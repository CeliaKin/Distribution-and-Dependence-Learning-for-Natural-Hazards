...
# Observation — three-component tail-biased design
...
n_moderate = 100
raw_mod = rng_obs.beta(1, 0.4, size=(n_moderate, number_of_dimension))
obs_moderate = (1.0 - raw_mod * 0.96).astype(np.float32)

n_extreme = 200
raw_ext = rng_obs.beta(3, 1, size=(n_extreme, number_of_dimension))
obs_extreme = (0.96 + raw_ext * 0.039).astype(np.float32)

obs_u = np.vstack([obs_bulk, obs_moderate, obs_extreme])
number_observation_points = len(obs_u)  # 500 total
...
ax1.plot(thresholds, chi_neural, '--',color='#2a9d8f', lw=2, label='Neural Copula (tail-biased)')
ax1.axhline(gumbel.upper_tail_dep, color='#e63946', ls='--', alpha=0.4)
ax1.axhline(t_cop.tail_dep,        color='#1d3557', ls='--', alpha=0.4)
...
ax2.plot(thresholds, chibar_neural, '--',color='#2a9d8f', lw=2, label='Neural Copula (tail-biased)')
ax2.axhline(1.0, color='k', ls='--', alpha=0.4, label='Asymptotic dep.')
...
plt.tight_layout()
plt.savefig("chi_comparison_tail_biased.png", dpi=150, bbox_inches='tight')
plt.show()
print("Saved → chi_comparison_tail_biased.png")
